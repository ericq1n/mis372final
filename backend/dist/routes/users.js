...
import { validateState, validatePhone, validateZipCode, validateDateOfBirth, } from '../utils/validators.js';
...
function applyUserUpdates(user, body) {
    ...
    if (phone !== undefined) {
        if (!validatePhone(phone))
            return 'phone must be a valid 10-digit US phone number';
    ...
    if (zipCode !== undefined) {
        if (!validateZipCode(zipCode))
            return 'zipCode must be 5 digits or ZIP+4';
